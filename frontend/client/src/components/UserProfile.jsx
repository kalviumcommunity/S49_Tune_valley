...
function UserProfile() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const Response = await axios.post('http://localhost:8000/postUserData/login', formData);
      console.log(Response.data);
      console.log('username0001', formData.username);
      Cookies.set('useremail', formData.username);
      ...
      console.log("cookie", Cookies.get('username')); // Logging the cookie value
  ...
  const handleSignOut = () => {
    Cookies.remove('username'); // Remove the 'useremail' cookie
      ...
      {!Cookies.get('useremail') ? (
        <div>
          ...
          <form onSubmit={handleSubmit}>
            <label>Name:</label>
            <input type="text" id="username" username="username" value={formData.name} onChange={handleChange} required />
            <label>Email:</label>
